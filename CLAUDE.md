...
## 16. App versioning rules

### Version format
The app uses **semantic versioning**: `MAJOR.MINOR.PATCH`.

- `MAJOR` — breaking changes or major product milestones (e.g. 1.0.0 = first public release).
- `MINOR` — new features that are backwards-compatible (e.g. new screen, new delivery channel).
- `PATCH` — bug fixes, copy changes, non-breaking improvements.

### Where versions are stored
Version must be kept in sync across all three locations:

| File | Field |
|------|-------|
| `apps/mobile/app.json` | `expo.version` |
| `apps/mobile/app.json` | `expo.android.versionCode` (integer, increment on every release) |
| `apps/mobile/package.json` | `version` |

### Rules
- Every APK build intended for distribution must have a unique `versionCode`. Never reuse a `versionCode`.
- `version` (semver string) is user-facing. `versionCode` is an internal integer used by Android to determine upgrade eligibility.
- `versionCode` must always increment monotonically — even for patch releases and hotfixes.
- When bumping `MINOR` or `MAJOR`, reset `PATCH` to 0. Do not reset `versionCode`.
- The API does not need to match the mobile version, but breaking API changes that require a minimum app version must be documented.

### Suggested versioning workflow
1. Decide the release type (major / minor / patch).
2. Update `apps/mobile/app.json`: bump `expo.version` and increment `expo.android.versionCode` by 1.
3. Update `apps/mobile/package.json`: bump `version` to match.
4. Commit: `chore: bump version to X.Y.Z (versionCode N)`.
5. Push and trigger a build on Bitrise or GitHub Actions.
6. Tag the commit: `git tag vX.Y.Z && git push origin vX.Y.Z`.

### Current version baseline
- `version`: `1.0.0`
- `versionCode`: `1` (first distribution build)

When the agent bumps a version it must update all three locations atomically in a single commit and must never leave them out of sync.
